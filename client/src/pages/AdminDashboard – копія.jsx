    ...
    const renderNewsView = () => (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in">
            <div className="lg:col-span-2 bg-white p-6 shadow-xl rounded-xl border border-pink-200">
                <div className="flex justify-between items-center mb-6"><h3 className="text-2xl font-semibold text-pink-700">{editingNews ? '✏️ Редагування' : '📢 Публікація'}</h3>{editingNews && <button onClick={cancelEditNews} className="text-sm bg-gray-100 px-3 py-1 rounded">Скасувати</button>}</div>
                {message && <div className="p-3 bg-blue-50 text-blue-800 rounded mb-4">{message}</div>}
                <form onSubmit={handleSaveNews} className="space-y-4">
                    <div>
                        <label className="block font-medium text-gray-700 mb-2">Тип публікації</label>
                        <div className="flex gap-4">
                            <label className="flex items-center gap-2 cursor-pointer bg-blue-50 px-3 py-2 rounded-lg border border-blue-100"><input type="checkbox" checked={isNews} onChange={(e) => setIsNews(e.target.checked)} className="w-5 h-5 text-blue-600" /><span className="font-bold text-blue-800">📰 Новина</span></label>
                            <label className="flex items-center gap-2 cursor-pointer bg-pink-50 px-3 py-2 rounded-lg border border-pink-100"><input type="checkbox" checked={isAnnouncement} onChange={(e) => setIsAnnouncement(e.target.checked)} className="w-5 h-5 text-pink-600" /><span className="font-bold text-pink-800">📣 Анонс події</span></label>
                        </div>
                    </div>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Прив'язати до події</label>
                        <select value={newsEventId} onChange={e => setNewsEventId(e.target.value)} className="w-full p-3 border rounded-lg bg-white"><option value="">-- Без прив'язки --</option>{events.map(ev => (<option key={ev.event_id} value={ev.event_id}>{ev.title} ({new Date(ev.start_datetime).toLocaleDateString()})</option>))}</select>
                    </div>
            ...
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 h-fit max-h-[800px] overflow-y-auto">
                ...
                {newsList.map(n => (
                    <div key={n.news_id} className={`bg-white p-3 mb-3 rounded shadow-sm border flex flex-col gap-2 ${editingNews?.news_id === n.news_id ? 'ring-2 ring-orange-400' : ''}`}>
                        ...
                        <div className="flex gap-2 flex-wrap">{n.is_news && <span className="text-xs px-2 py-1 rounded text-white bg-blue-500">Новина</span>}{n.is_announcement && <span className="text-xs px-2 py-1 rounded text-white bg-pink-500">Анонс</span>}</div>
                        <h5 className="font-bold mt-1">{n.title}</h5>
                        <div className="flex gap-2 justify-end mt-2"><button onClick={() => startEditNews(n)} className="text-xs bg-blue-50 text-blue-600 border border-blue-200 rounded px-2 py-1">✏️</button><button onClick={() => handleDeleteNews(n.news_id)} className="text-xs bg-red-50 text-red-600 border border-red-200 rounded px-2 py-1">🗑️</button></div>
                    </div>
                ))}
            </div>
        </div>
