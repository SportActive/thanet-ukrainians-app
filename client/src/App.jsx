        ...
        <div className="min-h-screen flex flex-col bg-gray-100 font-sans text-gray-900">
            <header className="bg-white shadow-md sticky top-0 z-50">
                <div className="container mx-auto px-4 py-3 flex justify-between items-center max-w-7xl">
                    <div onClick={() => setCurrentPage('news')} className="cursor-pointer flex items-center gap-2 group"><div className="w-10 h-10 bg-indigo-600 rounded-lg flex items-center justify-center text-white text-xl shadow-lg shadow-indigo-200 transition group-hover:bg-indigo-700">🇺🇦</div><div><h1 className="text-xl font-bold text-gray-800 leading-tight group-hover:text-indigo-700 transition">Українці в Танеті</h1><p className="text-xs text-gray-500 font-medium">Community App</p></div></div>
                    <nav className="hidden md:flex items-center space-x-1"><NavButton target="news" label="Новини" icon="📰" /><NavButton target="calendar" label="Календар" icon="📅" /><NavButton target="about" label="Про нас" icon="ℹ️" />{canAccessAdmin && (<button onClick={() => setCurrentPage('admin')} className={`ml-2 px-4 py-2 rounded-lg font-medium border flex items-center gap-2 transition ${currentPage === 'admin' ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'border-transparent text-gray-600 hover:bg-gray-100'}`}>⚙️ Адмінка</button>)}<div className="h-6 w-px bg-gray-300 mx-2"></div>{!user ? (<button onClick={() => setCurrentPage('login')} className="px-5 py-2 rounded-lg bg-indigo-600 text-white font-semibold shadow-md hover:bg-indigo-700 transition transform hover:-translate-y-0.5">Вхід</button>) : (<div className="flex items-center gap-3"><span className="text-sm font-medium text-gray-700">Привіт, <span className="text-indigo-600 font-bold">{user.first_name}</span></span><button onClick={handleLogout} className="text-red-600 hover:bg-red-50 px-3 py-1 rounded-lg text-sm font-medium transition border border-transparent hover:border-red-100">Вихід</button></div>)}</nav>
                    <button className="md:hidden p-2 text-gray-600 hover:text-indigo-600 transition" onClick={() => setIsMenuOpen(!isMenuOpen)}><svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16"></path></svg></button>
                </div>
